...
class TimeZone extends Component {
  ...
  componentDidMount = () => {
    ...
    setTimeout(() => {
      this.props.fetchArticle('how-to-keep-up-with-different-timezones-around-the-world-using-these-free-apps');
    }, 1000);
  };
    ...
    for (var i = 0; i < allimages.length; i++) {
      if (!allimages[i].classList.contains("color-input-img")) {
        ...
        if (allimages[i].getAttribute("data-width")) {
          allimages[i].setAttribute(
            "width",
            allimages[i].getAttribute("data-width")
          );
        }
        ...
        if (allimages[i].getAttribute("data-height")) {
          allimages[i].setAttribute(
            "height",
            allimages[i].getAttribute("data-height")
          );
        }
  ...
  componentWillReceiveProps = nextProps => {
    if (nextProps.article) {
      ...
      setTimeout(() => {
        webClient
          .get("/api/article?serialNumber=" + next)
          .then(response => {
            var articleCategory = response.data.data[0].articleCategory;
            this.setState({
      ...
      if (cookies.get(nextProps.article.articleUrl)) {
        if (
          nextProps.article.articleUrl ===
          cookies.get(nextProps.article.articleUrl)
        ) {
          this.setState({
            isRated: true,
  ...
  handleRate = (e, { rating, maxRating }) => {
    ...
    if (this.props.article) {
      ...
      articleObj = {
        ...
        articleRatingHigh: this.props.article.articleRatingHigh
          ? this.props.article.articleRatingHigh
          : maxRating,
        articleRatingLow: this.props.article.articleRatingLow
          ? Number(this.props.article.articleRatingLow) + 1
          : 0,
    ...
    if (articleObj._id !== "" && articleObj._id) {
      webClient
        ...
        .then(() => {
          ...
          const cookies = new Cookies();
          cookies.set(articleObj.articleUrl, articleObj.articleUrl, {
            path: "/" + articleObj.articleUrl
          });
        })
        .catch(() => {
          console.log("Error");
  ...
  render() {
    const articleSlug = this.props.article
      ? this.props.article.articleSlug
      : "<h1> </h1>";
    const htmlToRender = this.props.article
      ? this.props.article.articleBody
      : '<div style="min-height:60vh;"><h1>fetching the article...</h1></div>';
    const metaTitle = this.props.article
      ? this.props.article.metaTitle + " | mushfiqWEB - mushfiqur's blog"
      : "mushfiqWEB - mushfiqur's blog";
    const metaDescription = this.props.article
      ? this.props.article.metaDesc
      : "mushfiqWEB - mushfiqur's blog";
    const metaUrl = this.props.article
      ? this.props.article.metaUrl
      : "https://mushfiqweb.com";
    const metaImage = this.props.article
      ? this.props.article.metaImage
      : "https://i.imgur.com/mH4iRva.png";
    const metaKeys = this.props.article
      ? this.props.article.metaKeys
      : "https://mushfiqweb.com/article.png";
    ...
    var jsonLD = {
      ...
      publisher: {
        "@type": "Organization",
        name:
          "mushfiqWEB.com - blog of a tech geek, javascript developer, photographer and music enthusiast",
        logo: {
          "@type": "ImageObject",
    ...
    const tagsArray = this.props.article.articleCategory
      ? this.props.article.articleCategory.split(",")
      : [];
    ...
    const articleStats = this.props.article
      ? readingTime(this.props.article.articleBody)
      : "";
    ...
    let newmetaImage = "https://i.imgur.com/0CIRqKq.png";
    let newmetaTitle = "How to keep up with different timezones around the world using these free apps";
    let newmetaUrl = "https://mushfiqweb.com/how-to-keep-up-with-different-timezones-around-the-world-using-these-free-apps";
    let newmetaDescription = "It's quite difficult to keep up with the time when you are traveling multiple timezones. Up first is World Time Buddy, which you’ll want to bookmark right away. It lets you view timezones in a horizontal view, and you can add up to four locations to your screen at once for free (adding more will cost you $2.99 monthly for 20 locations. Plus, it shows weekends, warns you when daylight savings time is a week away, and you add meetings to your Google Calendar when you sign in with an account.";

    return (
      <div>
        <Helmet>
            <title>{newmetaTitle}</title>
            <meta name="keywords" content={this.props.article.metaKeys} />
            <meta name="description" content={newmetaDescription} />
            <meta property="og:url" content="https://mushfiqweb.com/how-to-keep-up-with-different-timezones-around-the-world-using-these-free-apps" />
            <meta property="og:title" content={newmetaTitle} />
            <meta property="og:image" content={newmetaImage} />
            <meta property="og:description" content={newmetaDescription} />
            <meta
              property="og:site_name"
              content="mushfiqWEB - Mushfiqur's Blog"
            />
            <meta property="og:locale" content="en_US" />
            <meta property="og:type" content="website" />
            <meta property="twitter:url" content="https://mushfiqweb.com/how-to-keep-up-with-different-timezones-around-the-world-using-these-free-apps" />
            <meta name="twitter:card" content="summary" />
            <meta name="twitter:site" content="@mushfiqweb" />
            <meta name="twitter:creator" content="@mushfiqweb" />
            <meta name="twitter:title" content={newmetaTitle} />
            <meta name="twitter:description" content={newmetaDescription} />
            <meta name="twitter:image" content={newmetaImage} />
        ...
        <ReactPlaceholder
          ...
          type="text"
          ready={this.props.article ? true : false}
          rows={3}
        >
          <Header
            className="slide-in-fwd-top"
            as="h1"
            textAlign="center"
            style={{ margin: "0 auto" }}
          >
        ...
        <Segment style={StyleSheet.articleContent} color={this.props.accent}>
            <ReactPlaceholder
            ...
            >
              <div
                style={window.innerWidth > 850 ? metaStyleRow : metaStyleColumn}
                className="slide-in-fwd-top"
              >
                <div style={StyleSheet.articleMeta}>Tag(s): {tagCompo}</div>
                <div>
                  ...
                  <div style={timeStringStyle} className="Alegreya">
                    <span className="ant-divider" />
                    <strong>{Math.ceil(articleStats.minutes)}</strong> Minutes
                    To Read
                  </div>
                  <div style={timeStringStyle} className="Alegreya">
                    <span className="ant-divider" />
                    <strong>
                      {this.props.article.articleRatingAvg &&
                      this.props.article.articleRatingAvg !== "NaN"
                        ? this.props.article.articleRatingAvg
                        : "0"}
                    </strong>{" "}
                    Rating
            ...
            >
              <div>
                ...
                <div
                  style={{
                    display: this.props.article.introImage ? "flex" : "none",
                ...
                >
                  ...
                  <img
                    ...
                    src={this.props.article.introImage}
                  />
                </div>
                {Parser(htmlToRender)}
              </div>
            ...
            <Segment color={this.props.accent}>
              <Grid columns={2}>
                <Grid.Column>
                  <Segment>
                    <Label
                      className="Alegreya one-rem-font"
                    ...
                    <span>
                      {this.state.prevTitle.length > 58
                        ? this.state.prevTitle.substring(0, 58) + "..."
                ...
                <Grid.Column>
                  <Segment>
                    ...
                    <span style={{ marginLeft: "-45px" }}>
                      {this.state.nextTitle.length > 68
                        ? this.state.nextTitle.substring(0, 68) + "..."
            ...
            >
              {metaUrl.length ? (
                <SocialShareCompo
                  ...
                  metaImage={metaImage}
                  accent={this.props.accent}
                  ratingDoneMsg={this.state.ratingDoneMsg}
